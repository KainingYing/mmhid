...
action_dict = {'bend': 0, 'box': 1, 'handshake': 2, 'hifive': 3, 'hug': 4, 'kick': 5, 'pat': 6, 'push': 7, 'others': 8,
               'be_bended': 8, 'be_boxed': 8, 'be_kicked': 8, 'be_pated': 8, 'be_pushed': 8,
               'no_action': 8}  # all action classes

action_dict_asym = {'bend': 0, 'box': 1, 'handshake': 2, 'hifive': 3, 'hug': 4, 'kick': 5, 'pat': 6, 'push': 7,
                    'others': 8,
                    'be_bended': 9, 'be_boxed': 10, 'be_kicked': 11, 'be_pated': 12, 'be_pushed': 13,
                    'no_action': 8}  # all action classes
...
def get_train_test_seq():
    clsNames = [cls for cls in action_dict.keys() if
                cls not in ['be_bended', 'be_boxed', 'be_kicked', 'be_pated', 'be_pushed',
                            'others', 'no_action']]
...
def BIT_read_annotations(sname):  # path to all seqs, seq name for one video
    ...
    with open(path, mode='r', encoding='utf–8') as f:
        ...
        for l in f.readlines():
            values = l.split()
            if len(values) == 4:  # frame id and box number frame title
                action_names = []
                ...
                annotations[frame_id] = {
                    ...
                    'box_num': box_num,
                    'bboxes': []
                    # normalized coordinates of top-left and bottom-right corners, eg.[(a1,b1,c1,d1), ..., (a4,b4,c4,d4)]
                }
            else:  # action
            ...
            if index == box_num:  # reading info for a frame finished
                ...
                for i in range(box_num):
                    for j in range(box_num):
                        if i == j:
                            continue
                        if (acts[i] == acts[j] and acts[i] != 8) or (acts[i] > 8 and acts[j] < 8) or (
                                acts[j] > 8 and acts[i] < 8):  # 丑陋
                            annotations[frame_id]['interactions'][cc] = 1
                        cc += 1
                annotations[frame_id]['actions'] = [action_dict[c] for c in
                                                    action_names]  # transfer "be_..." classes into "no_action"
                index = 0
        ...
        for k, v in list(annotations.items()):  # fliter no interaction image
        ...
        if self.data_augmentation == True:
            if random_factor == 0:  # scaling + random color jittering
                ...
                rnd = np.random.randint(0, 100) * (
                        upper - lower) * 0.01 + lower  # get a random number in [lower, upper]
                if rnd <= 1:  # zoom in and crop
                    ...
                    lim = rnd - 1
                    img = Pad((int((rnd - 1) * img.size[0]), int((rnd - 1) * img.size[1])), fill=0,
...
if __name__ == "__main__":
    # A = BIT_read_annotations('kick_0035')
    # print(A)
    print(get_train_test_seq())
    print(get_train_test_seq_from_partition())
